...
function endGame(winner) { // handles the end of a game : no stopGame function inside
    ...
    scores.red == 0 ? (scores.blue == 0 ? room.sendChat("🏆 " + GKList[0].name + " and " + GKList[1].name + " kept a CS ! ") : room.sendChat("🏆 " + GKList[1].name + " kept a CS ! ")) : scores.blue == 0 ? room.sendChat("🏆 " + GKList[0].name + " GOAT ! ") : null;
	updateStats();
}
...
function handleInactivity() { // handles inactivity : players will be kicked after afkLimit
	...
	for (var i = 0; i < extendedP.length ; i++) {
		if (extendedP[i][eP.ACT] == 60 * (2/3 * afkLimit)) {
			room.sendChat("[PV] ⛔ @" + room.getPlayer(extendedP[i][eP.ID]).name + ",  hareket etmez veya mesaj göndermezseniz " + Math.floor(afkLimit / 6) + " saniye sonra kickleniceksin !", extendedP[i][eP.ID]);
		}
...
function updateRoleOnPlayerOut() {
		...
		if (players.length >= 2 * maxTeamSize && scores.time >= (5/6) * game.scores.timeLimit && teamR.length != teamB.length) {
			if (teamR.length < teamB.length) {
				if (scores.blue - scores.red == 2) {
					endGame(Team.BLUE);
					room.sendChat("🤖 Ragequit algılandı. Game ended 🤖");
	...
	if (inChooseMode) {
		...
		if (Math.abs(teamR.length - teamB.length) == teamS.length) {
			room.sendChat("🤖 Başka seçenek kalmadı, bu durumu halletmeme izin verin... 🤖");
...
function choosePlayer() {
	...
	if (teamR.length <= teamB.length && teamR.length != 0) {
		room.sendChat("[PV] Bir oyuncu seçmek için verilen listeye numarasını girin veya 'random' yazın.", teamR[0].id);
		timeOutCap = setTimeout(function (player) { room.sendChat("[PV] Acele et @" + player.name + ", seçim yapmamk için sadece " + Number.parseInt(chooseTime / 2) + " saniyen kaldı !", player.id); timeOutCap = setTimeout(function (player) { room.kickPlayer(player.id, "zamanında seçim yapmadın !", false); }, chooseTime * 500, teamR[0]); }, chooseTime * 1000, teamR[0]);
	}
	else if (teamB.length < teamR.length && teamB.length != 0) {
		room.sendChat("[PV] Bir oyuncu seçmek için verilen listedeki numarasını girin veya 'random' yazın..", teamB[0].id);
		timeOutCap = setTimeout(function (player) { room.sendChat("[PV] Acele et @" + player.name + ", seçim yapmak için sadece " + Number.parseInt(chooseTime / 2) + " saniyen kadldı !", player.id); timeOutCap = setTimeout(function (player) { room.kickPlayer(player.id, "zamanında seçim yapmadın !", false); }, chooseTime * 500, teamB[0]); }, chooseTime * 1000, teamB[0]);
	}
...
room.onPlayerJoin = function(player) {
	...
	if (localStorage.getItem(player.auth) != null) {
		if (JSON.parse(localStorage.getItem(player.auth))[Ss.RL] != "player") {
			...
			room.sendChat((JSON.parse(localStorage.getItem(player.auth))[Ss.RL] == "master" ? "Admin " : "Admin ") + player.name + " odaya giriş yaptı !");
		}
	}
}
...
room.onPlayerTeamChange = function(changedPlayer, byPlayer) {
	...
	if (getAFK(changedPlayer) && changedPlayer.team != Team.SPECTATORS) {
		...
		room.sendChat(changedPlayer.name + " şu anda AFK !");
		return;
	}
	updateTeams();
	if (room.getScores() != null) {
...
room.onPlayerChat = function (player, message) {
	...
	if (["!help"].includes(message[0].toLowerCase())) {
		room.sendChat("[PV] 🚧👷Şu anda yapım aşamasında👷🚧", player.id);
		...
		if (players.length != 1 && player.team != Team.SPECTATORS) {
			if (player.team == Team.RED && streak > 0 && room.getScores() == null) {
				...
				room.sendChat("Takımdayken AFK olamazsın !", player.id);
				return false;
			}
		}
		...
		room.sendChat(player.name + (getAFK(player) ? " şu anda AFK !" : " AFK değil !"));
		...
		if (cstm == "[PV] Mute List : ") {
			room.sendChat("[PV] Sustutulan listesinde kimse yok !", player.id);
			return false;
		}
		cstm = cstm.substring(0, cstm.length - 2);
		cstm += ".";
			...
			if (!Number.isNaN(Number.parseInt(message[1])) && message.length > 1) {
				...
				if (message[2].length > 1 && message[2][0] == "#") {
					...
					if (!Number.isNaN(Number.parseInt(message[2])) && room.getPlayer(Number.parseInt(message[2])) != null) {
						...
						room.sendChat(room.getPlayer(Number.parseInt(message[2])).name + " susturuldu " + (timeOut / 60000) + " minutes!");
					}
				}
			}
				...
				if (message[1].length > 1 && message[1][0] == "#") {
					...
					if (!Number.isNaN(Number.parseInt(message[1])) && room.getPlayer(Number.parseInt(message[1])) != null) {
						...
						room.sendChat(room.getPlayer(Number.parseInt(message[1])).name + " 3 dakika susturuldu!");
					}
				}
			}
		}
		...
		if (player.admin && message.length >= 2) {
			if (message[1] == "all") {
				...
				room.sendChat("Sessize alınanlar temizlendi.");
			}
			else if (!Number.isNaN(Number.parseInt(message[1])) && room.getPlayer(Number.parseInt(message[1])) != null && getMute(room.getPlayer(Number.parseInt(message[1])))) {
				setMute(room.getPlayer(Number.parseInt(message[1])), false);
				room.sendChat(room.getPlayer(Number.parseInt(message[1])).name + " susturulma bitti !");
			}
			else if (Number.isNaN(Number.parseInt(message[1]))) {
				if (message[1].length > 1 && message[1][0] == "#") {
					message[1] = message[1].substring(1, message[1].length);
					if (!Number.isNaN(Number.parseInt(message[1])) && room.getPlayer(Number.parseInt(message[1])) != null && getMute(room.getPlayer(Number.parseInt(message[1])))) {
						setMute(room.getPlayer(Number.parseInt(message[1])), false);
						room.sendChat(room.getPlayer(Number.parseInt(message[1])).name + " susturulma kaldırıldı !");
					}
				}
			}
		}
		...
		if (player.admin) {
			if (message.length == 1) {
				slowMode = 5;
				room.sendChat(slowMode + " Yavaş mod etkin !");
			}
		...
		if (player.admin) {
			slowModec!= 0 ? room.sendChat(" Yavaş mod kapatıldı. ") : null;
			slowMode = 0;
		}
	}
		...
		if (player.admin) {
			if (message.length == 1) {
				room.clearBans();
				room.sendChat("Ban listesi temizlendi !");
				banList = [];
			}
			if (message.length == 2) {
				if (!Number.isNaN(Number.parseInt(message[1]))) {
					if (Number.parseInt(message[1]) > 0) {
						...
						if (banList.length != banList.filter((array) => array[1] != ID)) {
							room.sendChat(banList.filter((array) => array[1] == ID)[0][0] + " ban cezası kaldırıldı !");
						}
		...
		if (player.id == teamR[0].id || player.id == teamB[0].id) { // we care if it's one of the captains choosing
			if (teamR.length <= teamB.length && player.id == teamR[0].id) { // we care if it's red turn && red cap talking
				if (["top", "auto"].includes(message[0].toLowerCase())) {
					...
					clearTimeout(timeOutCap);
					room.sendChat(player.name + " rastgele seçim yaptı !");
					return false;
				}
					...
					if (Number.parseInt(message[0]) > teamS.length || Number.parseInt(message[0]) < 1) {
						room.sendChat("[PV] Numara geçersiz !", player.id);
						return false;
					}
					else {
...
room.onTeamGoal = function(team) {
	...
	if (lastPlayersTouched[0] != null && lastPlayersTouched[0].team == team) {
		if (lastPlayersTouched[1] != null && lastPlayersTouched[1].team == team) {
			room.sendChat("⚽ " + getTime(scores) + " Golü atan " + lastPlayersTouched[0].name + " ! Asisti yapan " + lastPlayersTouched[1].name + ". Topun hızı : " + ballSpeed.toPrecision(4).toString() + "km/h " + (team == Team.RED ? "🔴" : "🔵"));
		...
		room.sendChat("😂 " + getTime(scores) + " Kendi kalesine atan enayi " + lastPlayersTouched[0].name + " ! Şut hızı : " + ballSpeed.toPrecision(4).toString() + "km/h " + (team == Team.RED ? "🔴" : "🔵"));
...
room.onPlayerAdminChange = function (changedPlayer, byPlayer) {
	if (getMute(changedPlayer) && changedPlayer.admin) {
		room.sendChat(changedPlayer.name + " susturlma bitti.");
		setMute(changedPlayer, false);
	}
	if (byPlayer.id != 0 && localStorage.getItem(getAuth(byPlayer)) && JSON.parse(localStorage.getItem(getAuth(byPlayer)))[Ss.RL] == "admin") {
		room.sendChat("Bir oyuncuya yönetici adını verme izniniz yok !", byPlayer.id);
